...
**The self-scoring demo (`npm run demo:movement`) — a principle worth reusing.** The most
persuasive number in the README (3 of 3 planted regressions found, 0 of 7 innocents accused) was
also its least reproducible, because the constructed history had been retired. Retiring it was
right *as a CI fixture* (8 min, 367 MB — all of it Next.js's compiler); as an **opt-in script** the
cost is the verifier's to pay, and rebuilt on the tiny-fixture pattern it runs in **under 10
seconds**. It reads `trend --moves --json`, compares the named SHAs against the planted ones, and
prints `MISMATCH` if they diverge: **it is not a demonstration that supports the claim, it is the
claim, executed.** The README's most persuasive line became its most checkable one — which is the
standard the README sets for everyone else. It also renders the *not judged* line in the same
output, making refusal 4 visible rather than described.

**Publish prep findings (2026-08-25).** The clean-room test caught the bug it exists to catch:
`files: ["dist"]` would have published **without `action.yml`** — the manifest whose `runs.main`
points at the action entry — and without `LICENSE`. Both pass every in-repo test and break for
every user. Final whitelist: `["dist", "!dist/**/*.map", "action.yml", "README.md", "LICENSE"]`;
tarball 316 files, 263.5 kB packed. Sourcemaps excluded on an observable, not a preference: `dist`
is plain unminified `tsc` output with real function names, and maps pointing at a `src/` the
package doesn't ship would resolve to nothing — worse than no maps. The workflow template is
compiled into `dist` (proven in the clean room, not assumed). `exports` didn't expose
`./package.json`, which never affected the tool but would have broken consumers reading the
manifest by subpath.

**The most encouraging result was unplanned: it ran on Next.js 16.3.2** — a full major version
ahead of the 15.1.3 fixture — with detection, build, serve, Lighthouse and SSG exclusion all
working. First evidence that detection is not fitted to our own fixture's version.

**Ordering catch (2026-08-25), the one that would have inverted the scrub:** 19 commits — the
guards, M10, all of M11, the anonymisation, the README, the LICENSE — existed only locally while
`origin/main` still sat at the unscrubbed commit. Flipping visibility first would have published
the project name in 16 files and personal paths in 4, with the anonymisation living on one laptop.
*A second, subtler instance of the same ordering problem:* the published dashboard predated
per-class relevance (5 Chrome-driven breaks on `build_time`), so Pages would have served an
artefact contradicting the README that points at it. **Both fixed by the same push-first order.**
After the push: `build_time` shows 1 break (the 0.5.0→0.6.0 bump), `lcp:/` still shows 4 — Chrome
splits only what Chrome measured, visible in the live artefact. First full post-M9 stack run in CI:
clean, no stranded cache, no spurious protocol break, schema 2.2 with the build block.

*Post-launch fix queued:* record mode still writes `analysis: {outcome: "skipped"}` where the M11
step-1 contract says `not_applicable` — the same conflation, surviving in the one place the step-1
audit didn't reach because **the human surfaces are correctly silent and only the JSON is wrong**.
Two lines; deliberately not done mid-launch, since it changes a published contract field.

**The byte-identity proof closed — all three legs (2026-08-25).** Served by Pages, committed on
`perf-data`, and rendered locally: 258,392 bytes and sha256 `3d9dc6deca44` in all three. CI
generates it, the branch stores it, Pages serves it, and `driftwatch dashboard` reproduces it
offline from the same branch state. The M5 leg deferred since then holds, with no qualification. It
works because `dashboard-command` takes `generatedAt` from the latest entry's timestamp rather
than the wall clock — a choice made at M5 for exactly this reason.

**A pattern named (second instance): reporting an intention as an accomplishment.** The README's
`demo:movement` reproduction line was described to planning as written when it was only drafted —
so the most persuasive number in the README was, for two more steps, the one claim a reader could
not check, which is precisely what the script was built to prevent. The first instance was M9's
"the fix is live" when it had only ever run through a different entry point. **Both were caught by
...
**Marketplace prep (2026-08-25).** Branding revised on reasoning, not taste: icon `trending-up` →
`activity` — *in performance terms "up" is the bad direction*, so a growth arrow is ambiguous at
best and celebratory at worst, while a pulse line says **measuring** and is direction-neutral, which
matches a tool that reports improvements and regressions in the same table. Colour orange → blue:
alarm colours misrepresent the everyday experience, since **"no significant change" is the normal
outcome**. `author` corrected from the tool's own name to the person in the LICENSE. Marketplace
name `driftwatch` verified free **with controls in both directions** (a name that exists → 200, a
nonsense name → 404) — the method the npm episode taught. Categories: Continuous integration
(primary), Code quality.

**Live: Marketplace https://github.com/marketplace/actions/driftwatch-performance · release
https://github.com/AhmedNaguib20/Driftwatch/releases/tag/v0.6.0**

**The onboarding bug — third instance of a named class.** `init --github` has, since M3, generated
a workflow that **cannot work for anyone but this repo**: `npm ci && npm run build` (which builds
the user's project, not the action) followed by `uses: ./` (which looks for an `action.yml` the user
doesn't have). Invisible because this repo is the only place it was ever run, and our copy is
correct *for us*. **Same shape as `files: ["dist"]` and `DEFAULT_KEY_METRICS`: passes every in-repo
check, fails for every user.** The class deserves a standing question — *what else is only ever
exercised here?* — and a clean-room habit for generated artefacts, not just for the package.

Fix: `renderWorkflow()` branches on whether the target is this repo (by package name), emitting
`uses: ./` here and the published action elsewhere. **Pinned to the exact tag, never a floating
`@v0`** — and the reason is stronger than general reproducibility: `DRIFTWATCH_VERSION` feeds the
protocol hash, so a floating major would let an upgrade **silently split the user's trend data**
into a new segment they did not cause and cannot explain. Pinning makes the break deliberate and
dated, exactly as the Chrome pin two lines above already does.

**Fourth instance, and the sharpest: the published Action cannot execute.** GitHub Actions run
**from the repository at the referenced ref, not from npm** — and `action.yml` points at
`dist/adapters/github/action-entry.js` while `dist/` is gitignored. So `uses:
AhmedNaguib20/Driftwatch@v0.6.0` fails on first use. Hidden because our own workflow uses
`uses: ./` after building `dist/` in the runner: **the local form concealed that the published form
has no `dist/` at all.** The npm package is unaffected (its tarball does contain `dist/`, verified
in two clean rooms); this is specifically the Action-by-tag path.

*Decided:* `dist/` stays gitignored on `main` and a **release workflow force-adds it for the tagged
commit only** — day-to-day history stays clean while the tag is self-contained, which is what the
Action contract requires. Bundling to one file was rejected: it reverses the plain-`tsc` sourcemap
decision and makes the published artefact opaque, which sits badly with a product whose argument is
inspectable methodology. **The release process must be a workflow, not a remembered step** — "build
and commit dist before tagging" is exactly the kind of instruction that fails once and silently.

*And the listing:* a discoverable listing pointing at an Action that errors on first use is worse
than no listing. Fixed forward rather than delisted — `v0.6.1` on both npm and the tag (the
`init --github` fix warrants the bump on both, and `DRIFTWATCH_VERSION` is in the protocol hash, so
the two paths must not report different versions), with the broken `v0.6.0` release and tag
deleted. npm `0.6.0` stays: the CLI path works and unpublishing breaks anyone who installed.

**The release gate stopped its own first release — and the class got bigger.** Run
`32871573724` failed at "the suite must pass before anything is tagged": no tag, no release. Two
failures, both tests that only ever passed on one machine:
- `baseline.test.ts` overrode `arch: 'x64'` against a fixture reading `process.arch`. On an arm64
  Mac the override means something; **on an x64 runner the test compared a hash to itself and
  would have passed silently while proving nothing.** A test that asserts nothing but reads green
  is the same family as the superseded implementation with a passing test.
- `replay.test.ts` — picocolors emits ANSI on CI and suppresses it through a local non-TTY pipe, so
  an escape sequence sat mid-label. Swept the class rather than fixing the instance: the whole suite
  now runs green under `FORCE_COLOR=1`, the condition CI creates and a local pipe hides.

**Class restated: not "what is only exercised in this repo?" but "what is only exercised in this
execution context?"** — a single context treated as universal. Six instances now. The surface is
...
**Fourth release refusal, deepest yet: `dist/` was the missing piece, `node_modules/` was the piece
under it.** The published Action started and died in half a second on `ERR_MODULE_NOT_FOUND: yaml`.
**GitHub never installs dependencies for a JavaScript action** — it checks out the tag and runs
`runs.main` as-is. Everything upstream had worked: the published CLI detected a real Next.js 16.3.3
app, wrote a workflow pinned to `@v0.6.1`, and both jobs launched.

**The guard failed for a nameable reason: it checked presence, not executability.** *"A guard that
checks for a file is checking the artefact's spelling."* Same shape as the fabricated test count —
inspecting instead of running. **Principle: guards execute the artefact, they do not inspect it.**

*Decided — option C: the Action becomes a composite that runs the published npm package*
(`npx --yes @ahmednaguib/driftwatch@<version>`). Chosen on doctrine, not convenience: **hard rule 7
says the CLI is the product and CI is one consumer of it**, yet CI had been consuming a *second*
artefact — a tag carrying a hand-built copy of `dist` — and the two can diverge, which is exactly
what broke twice. Under C the Action runs the exact bytes a user runs locally, `dist/` never needs
committing anywhere, and the release workflow gets simpler rather than cleverer. Rejected: (A)
force-adding production `node_modules` — **measured at 181 MB / 13,665 files**, downloaded on every
consumer's every job, Lighthouse's transitive tree being 90% of it: technically correct,
practically hostile; (B) bundling — Lighthouse resists it (dynamic requires, worker assets) and a
subtly-broken bundle is the worst failure this project could ship.

*Consequences:* npm publish must **precede** the tag, and the workflow refuses to tag a version the
registry does not yet serve. The new guard runs `npx @ahmednaguib/driftwatch@<version> --version`
in an empty directory and reads what it prints.

**THE GATE IS GREEN (2026-08-25).** A repo that is not ours, a real Next.js 16.3.3 app from
`create-next-app`, a workflow written by `npx @ahmednaguib/driftwatch@0.6.2 init --github`, running
`uses: AhmedNaguib20/Driftwatch@v0.6.2`. Nothing local, nothing hand-edited. On a planted 347 KB
dataset: client bundle +35.4%, build output +17.0%, LCP +12.6%, transfer +14.3% — **four metrics
caught it, three correctly stayed quiet**, install time refused the delta on cache-state asymmetry
(§5.1 sixth), the SSG route named why it was skipped, the check run was neutral rather than
failing, the keyless tier line appeared once, and the footer identified the build. It behaved
exactly as designed on a machine that had never seen it.

**Final tally: six release refusals, five real defects, and one false alarm that found a sixth
defect anyway.** Two more principles came out of the last two:
- *Verification must run the same checks as the gate it verifies against.* The fifth refusal was a
  typecheck error invisible to a local loop of lint + tests while the gate ran lint + typecheck +
  test. The local command and the gate command are now **one command**.
- ***A negative assertion nothing can violate is a comment that runs.*** The sixth refusal was the
  guard's own bug — grepping for a literal version that can never appear because `npx` interpolates
  `$VERSION` — and beside it sat a test asserting "no written-out version anywhere", true of every
  possible version of that file.
Two more found by *running* rather than reading: `--version` prints build identity after the number
(comparing the whole line to a bare version would have failed a correct package), and **npm caches
package metadata**, so a cache written before a version existed answers "no such version" for one
the registry is serving — hence `--prefer-online` on both the guard and `action.yml`.

**The Marketplace-visitor path — a third route nobody had walked, and it found a live crash.**
`init --github` always writes the env block, so only a visitor pasting the Marketplace snippet can
reach the missing-token branch; that branch was honest but gave no remedy while the fetch-depth
failure two steps earlier shipped exact YAML. Beyond it sat a crash on **the most common PR shape
there is — one that adds a route**: a metric measured on the current side with no row at base fell
through a `!` assertion, computing `(current − null) / null × 100 = Infinity`.

Two lessons, and the second is bigger than the crash:
- ***The type system does not survive JSON.*** The field is declared `number`, so `Infinity` type-
  checks — and `JSON.stringify` writes it as `null`. The adapter received `verdict: regressed,
  exceedsThreshold: true, percent: null` and died. A contract boundary that serializes is a place
  where types stop being guarantees.
- **A metric with no baseline was reported as a regression.** Nothing was measured against it, so
  there is nothing to regress from — **rule 3 with the sign flipped**. And `skippedReason()` names
  this exact case in a comment ("a route added on this branch, say") while handling only the
  *skipped* half; measured-and-new reached production. ***A comment that names a case is not
  handling it.***

Fixed in core where the claim is made, with `formatPercent` made total in the adapter *and* the
CLI — which had the same shape and printed `+Infinity%` rather than crashing, i.e. quietly wrong,
which hard rule 7 says is the same bug.

*Also found:* `npm pack` runs `prepare` and **`--ignore-scripts` does not stop it**, so a packaging
test fired a second `tsc` into the `dist/` the suite was using. **A test whose side effects race the
harness is flaky by construction, exactly as one that races the clock is.**

**The visitor journey, walked byte-exact: four round trips, and only the first failure looked like
one.** The Marketplace snippet is three lines — no `with:`, no `env:`, no `permissions:` — so a
visitor is *guaranteed* to hit every prerequisite `init --github` writes for them. Run 1 failed in
16s with exact YAML (good). Runs 2 and 3 were **green ticks that silently did nothing**: a
regression measured and never delivered, its only trace a `console.error` in a collapsed step log —
**silence dressed as success, the worst available outcome for this project** — each costing 3.5
minutes to discover, and the last speaking in raw GitHub JSON rather than remedies.

*Two decisions:*
1. **One preflight, all prerequisites, before measuring.** Base reachability, token presence, and a
   permissions probe (a GET that 403s is the probe) are knowable in ~15 seconds. Report every
   missing one in a single stanza and fail once — four round trips at 3.5 minutes becomes one at
   fifteen seconds.
2. **A measured regression that could not be delivered must not be green.** This refines
   `publishResult` never failing the user's CI (M3 step 2) rather than reversing it: that rule was
   designed for **flakiness** — an API down or rate-limited — where staying green is right. A
   missing token or missing permission is **configuration**: it will never self-heal, and it is
   indistinguishable from success to whoever reads the PR. **The precedent already exists** — M3
   step 3 ruled that warn-only covers *measurement verdicts, not setup failures*, which is why the
   fetch-depth preflight exits 1. A missing token is a setup failure, so it exits 1 too. Transient
   delivery failure keeps the warning-and-green behaviour.
...
installed); visibility is far more reversible on a repo nobody has seen, and the package page links
back to a repo that should already resolve.

**Version at launch stays `0.6.0`** — 0.x honestly signals a pre-1.0 API, the number matches the
work behind it, and `DRIFTWATCH_VERSION` feeds the protocol hash, so resetting it would strand
caches for no reason. 1.0.0 would claim a stability that a tool with no users has not earned.
