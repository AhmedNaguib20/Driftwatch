...
export interface TriageOutput {
  ...
  readonly outOfDiffHints?: readonly string[]
}

export interface DeepOutput {
  /** Deep is the only stage allowed to conclude the diff does not explain the regression. */
  readonly explainsRegression: boolean
...
export const validateTriage: Validator<TriageOutput> = (raw) => {
  ...
  if (
    r.outOfDiffHints !== undefined &&
    (!Array.isArray(r.outOfDiffHints) || !r.outOfDiffHints.every((h) => typeof h === 'string'))
  ) {
    return { ok: false, problem: '"outOfDiffHints" must be an array of strings when present' }
  }
...
export const validateDeep: Validator<DeepOutput> = (raw) => {
  const r = raw as Record<string, unknown>
  if (typeof r?.explainsRegression !== 'boolean') {
    return { ok: false, problem: '"explainsRegression" must be a boolean' }
  }
